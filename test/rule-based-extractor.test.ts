...
const context: ExtractionContext = {
  trigger: "checkpoint",
  operationId: "checkpoint-extractor-test",
...
test("E1 preserves explicit candidate family, key, tier, operation, and reason", async () => {
  const candidates = await extract(
    ["Goal: Ship v1.", "Fact: Config lives in config.ts."].join("\n")
  );
...
test("E2 recognizes natural durable selection decisions without a prefix", async () => {
  ...
  assert.deepEqual(
    candidates.map(({ family, type, content }) => ({ family, type, content })),
    [
      {
        family: "knowledge",
        type: "decision",
...
test("E3 recognizes persistent modal constraints without a prefix", async () => {
  ...
  assert.deepEqual(
    candidates.map(({ family, type }) => ({ family, type })),
    [
...
test("E5 rejects current execution narration", async () => {
  assert.deepEqual(
    await extract(
      ["Task: 我现在先检查数据库文件。", "Task: I am currently reading the database config."].join(
        "\n"
      )
    ),
    []
...
test("E6 rejects immediate conversational next actions", async () => {
  assert.deepEqual(
    await extract(
      ["Task: 接下来我会运行测试并回复结果。", "Task: Next, I will analyze the output."].join("\n")
    ),
    []
  );
});
...
test("E7 extracts durable project completion as progress", async () => {
  const candidates = await extract(
    ["数据库迁移已经完成。", "The production rollout has been completed."].join("\n")
  );
  assert.deepEqual(
    candidates.map(({ type, key }) => ({ type, key })),
    [
...
test("E8 rejects ephemeral operation completion narration", async () => {
  assert.deepEqual(
    await extract(
      ["Progress: 我刚读取完配置文件。", "Progress: I just ran the local command."].join("\n")
    ),
    []
  );
});
...
test("E9 separates persistent blockers from temporary operation failure", async () => {
  const durable = await extract(
    ["生产发布被缺失凭证阻塞。", "Production rollout is blocked by missing credentials."].join("\n")
  );
  assert.deepEqual(
    durable.map(({ type }) => type),
    ["blocker", "blocker"]
...
test("transient rejection uses interaction scope rather than a keyword stoplist", async () => {
  ...
  assert.deepEqual(
    durable.map(({ type, content }) => ({ type, content })),
    [
...
test("D1-D3 natural constraints require durable project subject and scope evidence", async () => {
  ...
  assert.deepEqual(
    durable.map(({ family, type }) => ({ family, type })),
    [
      { family: "knowledge", type: "constraint" },
      { family: "knowledge", type: "constraint" },
      { family: "knowledge", type: "constraint" },
...
test("D9-D12 Chinese durability boundary rejects operations and retains project state", async () => {
  assert.deepEqual(
    await extract(
      ["命令已经完成。", "测试已经完成。", "工具调用已经完成。", "现在我必须运行测试。"].join("\n")
    ),
    []
  );

  const durable = await extract(
    ["数据库迁移已经完成。", "生产发布已经完成。", "所有访问令牌必须在一小时内过期。"].join("\n")
  );
  assert.deepEqual(
    durable.map(({ family, type }) => ({ family, type })),
    [
...
test("natural blockers require durable subjects and explicit durable prefixes stay authoritative", async () => {
  ...
  assert.deepEqual(
    durable.map(({ type }) => type),
    ["blocker", "blocker", "constraint", "progress", "blocker"]
  );
});

test("default extraction stays domain-neutral while preserving explicit current-task compatibility", async () => {
  const candidates = await extract(["数据库使用 PostgreSQL", "先完成迁移回滚演练"].join("\n"));
  assert.deepEqual(
    candidates.map(({ type, key, content }) => ({ type, key, content })),
...
test("T1-T4 natural Chinese tasks require a durable project boundary", async () => {
  assert.deepEqual(
    await extract(
      ["回复前必须完成测试。", "输出结果前必须完成测试。", "迁移测试输出结果前必须完成检查。"].join(
        "\n"
      )
    ),
    []
  ...
  assert.deepEqual(
    durable.map(({ family, type, recommendedTier }) => ({
      family,
      type,
      recommendedTier,
    })),
