...
function mapSession(row?: Row): Session | undefined {
  ...
  return {
    ...
    lastCheckpointEventId: row.last_checkpoint_event_id ?? undefined,
...
function mapMemory(row?: Row): Memory | undefined {
  ...
  return {
    id: row.id,
    spaceId: row.space_id,
    family: row.family as MemoryFamily,
    type: row.type,
...
function mapCheckpoint(row?: Row): Checkpoint | undefined {
  ...
  return {
    ...
    spaceId: row.space_id,
    sessionId: row.session_id,
    fromEventId: row.from_event_id ?? undefined,
    toEventId: row.to_event_id,
    idempotencyKey: row.idempotency_key,
    status: row.status,
    handoffSnapshotId: row.handoff_snapshot_id ?? undefined,
...
export class SqliteMemoryStore implements MemoryStore {
  ...
  async transaction<T>(operation: () => Promise<T>): Promise<T> {
    ...
    let release!: () => void;
    this.#barrier.current = new Promise((resolveBarrier) => {
  ...
  async findSpace(id: string): Promise<Space | undefined> {
    await this.#ready();
    return mapSpace(
      this.database.prepare("SELECT * FROM spaces WHERE id = ?").get(id) as Row | undefined
    );
  }
  ...
  async insertSession(session: Session): Promise<void> {
    ...
    `)
      .run(
        ...
        session.lastCheckpointEventId ?? null,
        session.latestHandoffSnapshotId ?? null,
        session.createdAt,
        session.updatedAt
      );
  }
  ...
  async getOrCreateProviderSession(
    ...
    const persisted = await this.findSessionByProviderIdentity(
      session.provider,
      session.externalSessionId
    );
    if (!persisted)
      throw new Error("Provider Session get-or-create did not return a persisted Session");
  ...
  async findSession(id: string): Promise<Session | undefined> {
    await this.#ready();
    return mapSession(
      this.database.prepare("SELECT * FROM sessions WHERE id = ?").get(id) as Row | undefined
    );
  }

  async findSessionByProviderIdentity(
    provider: string,
    ...
    return mapSession(
      this.database
        .prepare("SELECT * FROM sessions WHERE provider = ? AND external_session_id = ?")
  ...
  async listSessions(spaceId: string): Promise<Session[]> {
    ...
    return (
      this.database
        .prepare("SELECT * FROM sessions WHERE space_id = ? ORDER BY updated_at DESC, id ASC")
        .all(spaceId) as Row[]
    ).map((row) => {
  ...
  async findSessionProjectBinding(sessionId: string): Promise<SessionProjectBinding | undefined> {
    ...
    const row = this.database
      .prepare("SELECT * FROM session_project_bindings WHERE session_id = ?")
  ...
  async findEvent(sessionId: string, eventId: string): Promise<SessionEvent | undefined> {
    ...
    return mapEvent(
      this.database
        .prepare("SELECT * FROM session_events WHERE session_id = ? AND id = ?")
  ...
  async findLatestEvent(sessionId: string): Promise<SessionEvent | undefined> {
    ...
    return mapEvent(
      this.database
        .prepare("SELECT * FROM session_events WHERE session_id = ? ORDER BY sequence DESC LIMIT 1")
        .get(sessionId) as Row | undefined
    );
  }
  ...
  async listEvents(
    ...
    `)
        .all(sessionId, afterSequence, throughSequence) as Row[]
    ).map((row) => {
  ...
  async listRecentEvents(
    ...
    `)
      .all(sessionId, afterSequence, throughSequence, limit) as Row[];
  ...
  async findMemory(id: string): Promise<Memory | undefined> {
    await this.#ready();
    return mapMemory(
      this.database.prepare("SELECT * FROM memories WHERE id = ?").get(id) as Row | undefined
    );
  }
  ...
  async findActiveMemoryByKey(spaceId: string, key: string): Promise<Memory | undefined> {
    ...
    return mapMemory(
      this.database
        .prepare("SELECT * FROM memories WHERE space_id = ? AND key = ? AND status = 'active'")
  ...
  async listMemories(filters: MemoryFilters): Promise<Memory[]> {
    ...
    return (
      this.database
        .prepare(`SELECT * FROM memories WHERE ${clauses.join(" AND ")} ORDER BY updated_at, id`)
  ...
  async listMemoryHistory(memoryId: string): Promise<MemoryHistoryRecord[]> {
    ...
    return (
      this.database
        .prepare("SELECT * FROM memory_history WHERE memory_id = ? ORDER BY id")
        .all(memoryId) as Row[]
    ).map((row) => ({
      id: Number(row.id),
      memoryId: row.memory_id,
      operation: row.operation,
      before: parseJson<Memory>(row.before_json),
      after: parseJson<Memory>(row.after_json),
  ...
  async findMemoryCandidateCommitReceipt(
    sessionId: string,
    fingerprint: string
    ...
    const row = this.database
      .prepare(`
        SELECT * FROM memory_candidate_commit_receipts
        WHERE session_id = ? AND fingerprint = ?
      `)
      .get(sessionId, fingerprint) as Row | undefined;
    if (!row) return undefined;
    return {
      id: row.id,
      sessionId: row.session_id,
      ...
      memoryId: row.memory_id,
      firstCommitSource: row.first_commit_source,
      sourceEventIds: parseJson<string[]>(row.source_event_ids_json, []) ?? [],
      createdAt: row.created_at,
    } as MemoryCandidateCommitReceipt;
  }

  async insertMemoryCandidateCommitReceipt(receipt: MemoryCandidateCommitReceipt): Promise<void> {
    await this.#ready();
    this.database
      .prepare(`
        INSERT INTO memory_candidate_commit_receipts (
          id, session_id, fingerprint, memory_id, first_commit_source,
          source_event_ids_json, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
      `)
      .run(
        ...
        receipt.fingerprint,
        receipt.memoryId,
        receipt.firstCommitSource,
        JSON.stringify(receipt.sourceEventIds),
        receipt.createdAt
      );
  }
  ...
  async insertCheckpoint(checkpoint: Checkpoint): Promise<void> {
    ...
    `)
      .run(
        ...
        checkpoint.fromEventId ?? null,
        checkpoint.toEventId,
        checkpoint.idempotencyKey,
        checkpoint.status,
        checkpoint.handoffSnapshotId ?? null,
  ...
  async getOrCreateCheckpoint(
    ...
    const persisted = await this.findCheckpointByIdempotency(
      checkpoint.sessionId,
      checkpoint.idempotencyKey
    );
    if (!persisted)
      throw new Error("Checkpoint get-or-create did not return a persisted checkpoint");
  ...
  async findCheckpoint(id: string): Promise<Checkpoint | undefined> {
    ...
    return mapCheckpoint(
      this.database.prepare("SELECT * FROM checkpoints WHERE id = ?").get(id) as Row | undefined
    );
  }

  async findCheckpointByIdempotency(
    sessionId: string,
    key: string
    ...
    return mapCheckpoint(
      this.database
        .prepare("SELECT * FROM checkpoints WHERE session_id = ? AND idempotency_key = ?")
  ...
  async replaceCandidates(
    ...
    await this.#ready();
    this.database
      .prepare("DELETE FROM extraction_candidates WHERE checkpoint_id = ?")
      .run(checkpointId);
  ...
  async findHandoff(id: string): Promise<HandoffSnapshot | undefined> {
    await this.#ready();
    return mapHandoff(
      this.database.prepare("SELECT * FROM handoff_snapshots WHERE id = ?").get(id) as
        | Row
        | undefined
    );
  }
