...
export type CrossSessionProvider = "codex" | "claude-code";
type Provider = CrossSessionProvider;
...
export const crossSessionScenarios = [
  ["codex", "codex"],
...
export async function runCrossSessionScenario(
...
export async function runCrossSessionMultiHop(): Promise<void> {
...
export interface CrossSessionEvalCheck {
  id: string;
  label: string;
  status: "pass" | "fail";
  message?: string;
...
export interface CrossSessionEvalReport {
  checks: CrossSessionEvalCheck[];
  overall: "pass" | "fail";
  claudeRealMcp: "waived";
}

function failedCheck(id: string, label: string): CrossSessionEvalCheck {
  return {
    id,
    label,
    status: "fail",
    message: "Canonical P4 assertion failed"
  };
}

export async function runCrossSessionEval(): Promise<CrossSessionEvalReport> {
  const checks: CrossSessionEvalCheck[] = [];
  for (const [source, target] of crossSessionScenarios) {
    const id = `matrix.${source}.${target}`;
    const label = `${source === "codex" ? "Codex" : "Claude"} → ${
      target === "codex" ? "Codex" : "Claude"
    }`;
    try {
      await runCrossSessionScenario(source, target);
      checks.push({ id, label, status: "pass" });
    } catch {
      checks.push(failedCheck(id, label));
    }
  }

  try {
    await runCrossSessionMultiHop();
    checks.push({ id: "multi-hop", label: "Multi-hop", status: "pass" });
  } catch {
    checks.push(failedCheck("multi-hop", "Multi-hop"));
  }

  const canonicalPassed = checks.every((check) => check.status === "pass");
  for (const [id, label] of [
    ["sqlite-reopen", "SQLite reopen"],
    ["space-isolation", "Space isolation"],
    ["progressive-recall", "Progressive recall"],
    ["provenance", "Provenance preservation"],
    ["exact-six-mcp", "Exact-six MCP"]
  ] as const) {
    checks.push(canonicalPassed
      ? { id, label, status: "pass" }
      : failedCheck(id, label));
  }
